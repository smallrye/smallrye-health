 ...
 * @since 29/03/16
 *
 *        Needed because of the package private access in the MicroProfile Health TCK module.
 */
public abstract class TCKBase extends Arquillian {
    ...
    Response getUrlWellContents() {
        return getUrlContents(this.uri + "/health/well", false);
    }

    Response getUrlStartupContents() {
        return getUrlContents(this.uri + "/health/startup", false);
    }

    public Response getUrlAllCustomHealthContents() {
        return getUrlContents(this.uri + "/health/group", false);
    }

    public Response getUrlCustomHealthContents(String name) {
    ...
    public void assertSuccessfulCheck(JsonObject check, String expectedName) {
        assertCheckName(check, expectedName);
        verifySuccessStatus(check);
    }

    public void assertFailureCheck(JsonObject check, String expectedName) {
        assertCheckName(check, expectedName);
    ...
    public void verifySuccessStatus(JsonObject check) {
        Assert.assertEquals(
    ...
    public void verifyFailureStatus(JsonObject check) {
        Assert.assertEquals(
                check.getString("status"),
                "DOWN",
    ...
    public void assertOverallSuccess(JsonObject json) {
        Assert.assertEquals(
    ...
    public void assertOverallFailure(JsonObject json) {
        Assert.assertEquals(
                json.getString("status"),
                "DOWN",
