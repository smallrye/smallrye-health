...
find examples -depth 1 -type d | xargs -I{} mvn -pl {} build-helper:parse-version versions:set -DnewVersion=\${parsedVersion.nextMajorVersion}.0.\${parsedVersion.incrementalVersion}-SNAPSHOT
...
# change the release version to 11 for 4.0.x stream
sed -i 's/java-version: 8/java-version: 11/g' .github/workflows/release.yml
